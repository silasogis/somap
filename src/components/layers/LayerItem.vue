    ...
    <div class="px-3 pt-2.5 pb-1 flex items-center justify-between gap-2">
      ...
      <div class="flex items-center gap-1.5 flex-shrink-0">
        <span 
          ...
          :class="layer.isLocal ? 'bg-amber-100 text-amber-800 dark:bg-amber-950/60 dark:text-amber-300 border border-amber-300/60 dark:border-amber-700/50' : 'bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400'"
          :title="layer.isLocal ? 'Camada temporária carregada na sessão' : ''"
        >
          {{ layer.type }}
        </span>
        <button
          v-if="layer.isLocal"
          @click="handleRemove"
          class="p-0.5 text-gray-400 hover:text-red-600 dark:hover:text-red-400 rounded transition-colors"
          title="Remover esta camada da sessão"
        >
